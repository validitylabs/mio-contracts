...
const PORT = cnf.networks.coverage.port;
const web3 = new Web3(new Web3.providers.HttpProvider('http://' + cnf.networks.coverage.host + ':' + PORT));
...
function startServer() {
    const config = {
        ...
        // mnemonic: cnf.networks.coverage.mnemonic,
        port: 8555,
        locked: false,
        // debug: true,
        gasPrice: 1,
    ...
    ganache.listen(PORT, async (err) => {
        if (err) {
            ...
            if (process.env.verbose) {
                ...
                log.info('Host\t: http://' + cnf.networks.coverage.host + ':' + PORT);
